...
if menu == "🏠 1. ภาพรวมและสถิติ (Dashboard)":
        ...
        # 🌟 สเตปที่ 1: โหลดข้อมูลจาก 3 ตารางหลักมาเก็บไว้ก่อน
        try:
        ...
        # 🌟 สเตปที่ 2: สร้างการ์ดสรุปตัวเลข (KPI Metrics)
        ...
        # 🌟 สเตปที่ 3: แบ่งหน้าจอเป็น 2 ฝั่ง (ซ้าย: แจ้งเตือนซิม | ขวา: ภาระงาน)
        col_left, col_right = st.columns(2)

        # 🚨 ระบบสแกนซิมการ์ดหมดอายุ (Global Alert)
        with col_left:
            ...
            if not df_pm.empty and 'วันที่ซิมหมดอายุ' in df_pm.columns and 'ชื่อไซต์งาน' in df_pm.columns:
                ...
                if not sim_data.empty:
                    with st.container(height=300): # สร้างกล่องให้เลื่อน Scroll ได้
                        for _, row in sim_data.iterrows():
                            site = str(row['ชื่อไซต์งาน']).strip()
                            exp = str(row['วันที่ซิมหมดอายุ']).strip()
                            
                            # ถ้ามีคำว่า "หมด" ให้ขึ้นป้ายแดงเตือนภัย
        ...
        # 👷‍♂️ ระบบแสดงภาระงานลูกทีม (Workload)
        with col_right:
            ...
            if not df_task.empty and 'ผู้รับผิดชอบหลัก' in df_task.columns and 'สถานะงาน' in df_task.columns:
                ...
                if not active_df.empty:
                    # จัดกลุ่มนับจำนวนงานค้างของแต่ละคน
