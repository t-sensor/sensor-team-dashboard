...
# --- โค้ดเมนูอื่นๆ ที่อยู่ด้านล่าง ปล่อยไว้เหมือนเดิมครับ ---
    ...
    try:
        # ดึงข้อมูลจากแผ่น Task & Workload (ใช้ฟังก์ชัน load_sheet จากเมนู 1)
        df_tasks = load_sheet("Task & Workload")
        
        if not df_tasks.empty:
            # จัดการค่าว่าง (ป้องกัน Error)
            df_tasks['ผู้รับผิดชอบหลัก'] = df_tasks['ผู้รับผิดชอบหลัก'].fillna("")
            df_tasks['ผู้ช่วย'] = df_tasks['ผู้ช่วย'].fillna("")
            
            # ฟิลเตอร์: เอางานที่ CURRENT_USER เป็น "ผู้รับผิดชอบหลัก" หรืออยู่ในคอลัมน์ "ผู้ช่วย"
            my_tasks = df_tasks[
            ...
            if not my_tasks.empty:
                ...
                # ปรับแต่งตารางให้ดูง่ายขึ้น (ซ่อนคอลัมน์เวลาบันทึกเพื่อความสะอาดตา)
                display_cols = ['วันที่เข้าทำ (Scheduled Date)', 'ชื่อไซต์งาน', 'ชื่องาน / รายละเอียด', 'ประเภทงาน', 'สถานะงาน', 'ผู้ช่วย']
                # เช็คว่ามีคอลัมน์ครบไหมก่อนโชว์
                available_cols = [col for col in display_cols if col in my_tasks.columns]
        ...
        st.warning("กำลังรอข้อมูลงานแรกของคุณเข้าสู่ระบบครับ...")
