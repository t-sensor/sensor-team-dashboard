...
# 2. --- ส่วนแสดงตารางงานของตัวเอง ---
    ...
    st.write("รวบรวมสูตรคำนวณที่จำเป็นสำหรับงานหน้าไซต์ และคลังความรู้ของทีม Sensor")
    
    # สร้างแท็บเพื่อแยกเครื่องมือคำนวณ กับ คลังความรู้
    tab1, tab2 = st.tabs(["🧮 เครื่องมือคำนวณ (Delta T)", "📚 คลังความรู้ (Knowledge Base)"])
    
    # --- Tab 1: เครื่องมือคำนวณ Delta T ---
    with tab1:
        st.markdown("### ❄️ โปรแกรมคำนวณค่า Delta T (ระบบ Chiller Plant)")
        st.write("เครื่องมือช่วยคำนวณผลต่างอุณหภูมิน้ำเย็น (CHW) สำหรับประเมินประสิทธิภาพระบบปรับอากาศ")
        
        # กล่องรับค่าอุณหภูมิ
        col1, col2 = st.columns(2)
        with col1:
            chwrt = st.number_input("🌡️ อุณหภูมิน้ำกลับ (CHWRT - Return) °C", value=12.0, step=0.1)
        with col2:
            chwst = st.number_input("❄️ อุณหภูมิน้ำจ่าย (CHWST - Supply) °C", value=7.0, step=0.1)
        ...
        st.markdown("---")
        # แสดงผลลัพธ์ตัวใหญ่ๆ
        st.markdown(f"<h3 style='text-align: center; color: #008080;'>ผลลัพธ์ค่า Delta T = {delta_t:.2f} °C</h3>", unsafe_allow_html=True)
        
        # ระบบ AI วิเคราะห์ผลเบื้องต้น (อิงตามมาตรฐานทั่วไปที่ 5 องศาเซลเซียส)
        if delta_t >= 4.5 and delta_t <= 5.5:
            st.success("✅ **วิเคราะห์ผล:** ค่า Delta T อยู่ในเกณฑ์มาตรฐาน (การแลกเปลี่ยนความร้อนทำงานได้ดี)")
        elif delta_t > 0 and delta_t < 4.5:
            st.warning("⚠️ **วิเคราะห์ผล:** ค่า Delta T ต่ำกว่ามาตรฐาน (อาจเกิดภาวะ Low Delta T Syndrome หรือโหลดความร้อนในระบบน้อยกว่าที่ออกแบบไว้)")
        elif delta_t > 5.5:
            st.info("ℹ️ **วิเคราะห์ผล:** ค่า Delta T สูงกว่ามาตรฐาน (โหลดความร้อนในพื้นที่อาจจะสูงมาก หรืออัตราการไหลของน้ำ Flow Rate น้อยเกินไป)")
        else:
            st.error("🚨 **ข้อผิดพลาด:** อุณหภูมิน้ำจ่าย (Supply) สูงกว่าน้ำกลับ (Return) โปรดตรวจสอบเซ็นเซอร์อีกครั้ง!")

    # --- Tab 2: คลังความรู้เตรียมรอรับเนื้อหา ---
    with tab2:
        st.markdown("### 📚 ศูนย์รวมคู่มือและ Quiz ทดสอบ")
        st.info("💡 **เตรียมพบกับ:** ระบบดึงข้อสอบ (Quiz) และคู่มือการติดตั้งเซ็นเซอร์จาก Google Sheets ในเร็วๆ นี้ครับ")
        st.write("ถ้าคุณ Heart มีตารางคำถาม-คำตอบ (Quiz) ใน GSheet เราสามารถเอามาเชื่อมเพื่อให้ทีมงานกดทำแบบทดสอบเก็บคะแนนในหน้านี้ได้เลยครับ!")
else:
    st.title(menu)
