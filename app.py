...
# 2. --- ส่วนแสดงตารางงานของตัวเอง ---
    ...
    st.write("บันทึกประวัติการยืม-คืน สามารถ **กดเลือกอุปกรณ์พร้อมกันหลายชิ้น** ได้เลยครับ!")
    ...
    # 🔧 ลิสต์อุปกรณ์ส่วนกลางทั้งหมด (คุณ Heart สามารถพิมพ์แก้ชื่อ หรือเพิ่มอุปกรณ์ในวงเล็บนี้ได้เลยครับ)
    tool_options = [
        "สว่านแบตเตอรี่", "สว่านโรตารี่", "บันได 7 ขั้น", "บันได 5 ขั้น", 
        "เครื่องวัด Signal Loss", "มัลติมิเตอร์ (VOM)", "ชุดไขควงช่าง", 
        "ชุดประแจ", "เครื่องเป่าลม (Blower)", "สาย LAN 100m", "หัว RJ45 (กล่อง)"
    ]
    ...
    with st.form("tools_form"):
        ...
        with col1:
            ...
            # เปลี่ยนเป็น Multiselect ให้กดเลือกได้หลายอัน
            selected_tools = st.multiselect("🔧 เลือกอุปกรณ์ (กดเลือกได้หลายชิ้น)", tool_options)
            # ช่องเผื่อพิมพ์ของแปลกๆ ที่ไม่มีใน Dropdown
            other_tools = st.text_input("อุปกรณ์อื่นๆ (นอกเหนือจากด้านบน)", placeholder="พิมพ์เพิ่มได้ที่นี่")
        ...
        with col2:
            ...
            # เพิ่มช่องให้ระบุจำนวน
            quantity_note = st.text_input("📦 หมายเหตุจำนวน", placeholder="เช่น สว่าน 2 ตัว, สายแลน 3 กล่อง")
            
        submitted = st.form_submit_button("บันทึกข้อมูล", type="primary")
        
        if submitted:
            # รวมอุปกรณ์ที่เลือกจาก Dropdown และที่พิมพ์เองเข้าด้วยกัน
            all_tools = []
            if selected_tools:
                all_tools.extend(selected_tools)
            if other_tools:
                all_tools.append(other_tools)
                
            final_tools_str = ", ".join(all_tools)
            
            # ถ้าระบุจำนวนมาด้วย ให้เอาไปต่อท้ายชื่ออุปกรณ์
            if quantity_note:
                final_tools_str = f"{final_tools_str} ({quantity_note})"
            ...
            if final_tools_str:
                ...
                with st.spinner("กำลังบันทึกข้อมูล..."):
                    try:
                        response = requests.post(GAS_URL, data=json.dumps(payload))
                        if response.json().get("status") == "success":
                            st.success(f"บันทึกการ '{status}' เรียบร้อย! ✅ อุปกรณ์: {final_tools_str}")
                    except Exception as e:
                        st.error(f"ระบบขัดข้อง: {e}")
            else:
                st.warning("⚠️ กรุณาเลือกหรือพิมพ์ชื่ออุปกรณ์ ก่อนกดบันทึกครับ")
