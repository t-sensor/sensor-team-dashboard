...
# 2. --- ส่วนแสดงตารางงานของตัวเอง ---
    ...
    with tab1:
        try:
            ...
            if not df_learning.empty and 'หัวข้อ' in df_learning.columns:
                for index, row in df_learning.iterrows():
                    category = str(row.get('หมวดหมู่', 'ทั่วไป'))
                    topic = str(row.get('หัวข้อ', ''))
                    # ปรับเปลี่ยนการขึ้นบรรทัดใหม่จาก GSheet ให้แสดงบนเว็บได้สวยงาม
                    content = str(row.get('เนื้อหาแบบละเอียด', '')).replace('\n', '<br>')
                    img_link = str(row.get('ลิงก์รูปภาพประกอบ', ''))
                        ...
                        with st.expander(f"📖 [{category}] {topic}"):
                            ...
                            if img_link and img_link.startswith('http'):
                                try:
                                    ...
                                    st.write(f"🔗 [คลิกเพื่อดูรูปภาพประกอบ]({img_link})")
            else:
                st.info("ยังไม่มีข้อมูลในแผ่น Learning_Content ครับ")
        except Exception as e:
