...
# --- โค้ดเมนูอื่นๆ ที่อยู่ด้านล่าง ปล่อยไว้เหมือนเดิมครับ ---
    ...
    st.write("ตรวจสอบข้อมูลอุปกรณ์ที่ติดตั้ง, แผน PM และประวัติปัญหาก่อนเข้าหน้างานจริง")

    try:
        # โหลดรายชื่อไซต์งานทั้งหมดมาทำเป็นช่องค้นหา (Dropdown)
        df_master = load_sheet("Master_Site")
        site_list = df_master['ชื่อไซต์งาน (Process Work)'].dropna().unique().tolist()
        
        # 🔍 ช่องเลือกไซต์งาน
        selected_site = st.selectbox("🔍 ค้นหาหรือเลือกไซต์งานที่ต้องการดูข้อมูล:", site_list)
        
        st.markdown("---")
        st.subheader(f"📍 ข้อมูลสรุปของไซต์: {selected_site}")
        
        # สร้างแท็บ (Tabs) 3 หน้าในหน้าเดียว
        tab1, tab2, tab3 = st.tabs(["📡 อุปกรณ์ที่ติดตั้ง (Assets)", "🗓️ แผน PM (PM Plan)", "🚨 ประวัติงานและปัญหา (Issue Log)"])
        
        # --- Tab 1: อุปกรณ์ที่ติดตั้ง (ดึงจากแผ่น Asset_Sensor) ---
        with tab1:
            try:
                df_assets = load_sheet("Asset_Sensor")
                # ทำความสะอาดหัวคอลัมน์
                ...
                if not df_assets.empty and 'ชื่อไซต์งาน' in df_assets.columns:
                    ...
                    if not site_assets.empty:
                        # โชว์ตารางโดยซ่อนคอลัมน์ "ชื่อไซต์งาน" ไว้ เพราะรู้ชื่อไซต์อยู่แล้ว
                    ...
                    st.warning("ตาราง Asset_Sensor ยังไม่มีข้อมูล หรือขาดคอลัมน์ 'ชื่อไซต์งาน'")
            except Exception as e:
                st.error(f"ไม่สามารถโหลดข้อมูลอุปกรณ์ได้: {e}")
                
        # --- Tab 2: แผนงานประจำปี (ดึงจากแผ่น PM_Plan) ---
        with tab2:
            try:
                df_pm = load_sheet("PM_Plan")
                df_pm.columns = [str(c).strip() for c in df_pm.columns]
                
                # ค้นหาว่าคอลัมน์ไหนคือชื่อไซต์ (รองรับทั้งคำว่า 'ชื่อไซต์งาน' หรือ 'Process Work')
                site_col = None
                ...
                if site_col:
                    ...
                    if not site_pm.empty:
                        st.dataframe(site_pm, use_container_width=True, hide_index=True)
                    else:
                        st.info("ยังไม่มีแผน PM สำหรับไซต์นี้ในปีนี้ครับ")
                else:
                    st.dataframe(df_pm, use_container_width=True) 
            except Exception as e:
                st.error(f"ไม่สามารถโหลดข้อมูลแผน PM ได้: {e}")
                
        # --- Tab 3: ประวัติปัญหา (ดึงจากแผ่น Task & Workload) ---
        with tab3:
            try:
                ...
                if not df_tasks.empty and 'ชื่อไซต์งาน' in df_tasks.columns:
                    ...
                    if not site_tasks.empty:
                        # สร้างตัวฟิลเตอร์ให้กดดูเฉพาะงานที่มีปัญหาได้ง่ายๆ
                        ...
                        st.success("🎉 ไซต์นี้ยังไม่เคยมีประวัติการแจ้งซ่อมหรือเจอปัญหาเลยครับ ยอดเยี่ยมมาก!")
                else:
                    st.warning("ตาราง Task & Workload ยังไม่มีข้อมูล หรือขาดคอลัมน์ 'ชื่อไซต์งาน'")
            except Exception as e:
                st.error(f"ไม่สามารถโหลดข้อมูลประวัติงานได้: {e}")

    except Exception as e:
        st.error(f"ระบบขัดข้อง กรุณาตรวจสอบว่ามีแผ่น Master_Site ใน GSheet หรือไม่: {e}")
