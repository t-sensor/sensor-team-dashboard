...
# --- Tab 1: คลังความรู้ (ดึงจาก Learning_Content) ---
    ...
    try:
        ...
        if not df_docs.empty and 'หมวดหมู่' in df_docs.columns:
            # วนลูปสร้างการ์ดโฟลเดอร์ทีละหมวดหมู่
            for _, row in df_docs.iterrows():
                cat_name = str(row.get('หมวดหมู่', ''))
                desc = str(row.get('รายละเอียด', '-'))
                # รองรับทั้งชื่อคอลัมน์ใหม่และเก่า
                link = str(row.get('ลิงก์โฟลเดอร์', row.get('ลิงก์เอกสาร', ''))) 
                    ...
                    with st.container():
                        ...
                        with col1:
                            ...
                            if desc and desc.lower() != 'nan' and desc != '-':
                                st.write(f"ℹ️ {desc}")
                        with col2:
                            st.markdown("<br>", unsafe_allow_html=True) # ดันปุ่มลงมาให้ตรงกับข้อความ
